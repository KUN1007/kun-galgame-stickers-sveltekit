...
**已立项，已实现，等 Deploy。** infra 于 2026-09-08 交付了 B 档校验端点与 `sticker:read` scope（`96b689d0`），本站同日交付了面实现与 OpenAPI 契约；同日 infra 复核后把下游面的公开前缀改判为 **`/v2/<site>/*`**（PR #167），本仓已全面收敛到 `/v2/sticker`。剩下的是接线动作（面板 Deploy、真 key 冒烟、spec 注册），见 [03 §5](./03-wiring.md)。
